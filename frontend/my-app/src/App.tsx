    ...
    <main className="rally-shell">
      ...
      <section className="board" aria-label="Rally board columns">
        {Object.entries(boardData).map(([column, cards], columnIndex) => (
          <article className="lane" key={column} style={{ animationDelay: `${columnIndex * 0.09}s` }}>
              ...
              {cards.map((card) => (
                <article className="story-card" key={card.id}>
                  <div className="story-top">
                    ...
                    <span className={`priority-chip ${priorityClass[card.priority]}`}>{card.priority}</span>
                  </div>

                  <p className="story-title">{card.title}</p>

                  <div className="meta-row">
                    <span className={`kind-badge kind-${card.kind.toLowerCase()}`}>{card.kind}</span>
